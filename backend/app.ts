...
app.get('/dashboard', async (c) => {
  ...
  const data = await asUser(user.id, async (tx) => {
    ...
    const activity = await tx`select to_char(day, 'YYYY-MM-DD') as day, coalesce(sum(clicks), 0)::int as clicks from generate_series(current_date - interval '6 days', current_date, interval '1 day') day left join analytics.link_daily_stats s on s.day = day::date and exists (select 1 from links.links l where l.id = s.link_id and l.user_id = auth.current_user_id() and l.deleted_at is null) group by day order by day`
